...
fn main() {
    ...
    if let Some(command) = command {
        match command {
            Command::Compress {
                ...
                let compressed_bytes =
                    compress(&input_bytes, level).expect("Failed to compress file");
            ...
            Command::Encode {
                ...
                if output_type.has_texture() {
                    ...
                    let compressed =
                        compress(&bytes, compression_level).expect("Failed to compress");
        ...
        app.run().expect("Failed to run main window");
    }
}
